...
class FairBatch(Sampler):
    ...
    def __init__(self, model, x_tensor, y_tensor, z_tensor, batch_size, alpha, target_fairness, replacement = False, seed = 0):
        ...
        for tmp_yz in self.yz_tuple:
            # print('y', self.y_data == tmp_yz[0])
            # print('z', self.z_data == tmp_yz[1])
            self.yz_mask[tmp_yz] = (self.y_data == tmp_yz[0]) & (self.z_data == tmp_yz[1])
            # raise ValueError()
        # print('yz_mask',self.yz_mask[self.yz_tuple[1]].shape)
        # print('z_mask',self.z_mask[self.z_item[1]].shape)
        # Finds the index
        self.z_index = {}
        self.y_index = {}
        self.yz_index = {}
    ...
    def adjust_lambda(self):
        ...
        if self.fairness_type == 'eqopp':
            ...
            eo_loss = criterion ((F.tanh(logit)+1)/2, (self.y_data.reshape(-1,1)+1)/2)
